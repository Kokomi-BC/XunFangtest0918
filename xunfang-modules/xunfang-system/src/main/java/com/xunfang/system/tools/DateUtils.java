 ...
 * 
* @ClassName: DateUtils
* @Description: TODO(日期格式化工具类)
* @author 刘念
* @date 2019年7月25日 下午2:17:12
*
 */
public class DateUtils {
     ...
     */
    public static String dateParseToStrLong(Date dateDate){
        try {
            return dateToStrLong(dateDate);
        }catch (Exception e){
            return null;
        }
    }
	 ...
	 * @return
	 */
	public static int calculateTime(Date fromDate,Date toDate) {
	 ...
	 * @return
	 */
	public static Integer getRemainOneDay(Date currentDate) {
        ...
        return ((int) minutes)+1;//+1处理保证getRemainOneDay+getPastTimeOneDay闭环1440
    }
	/**
	 * 计算时间离日期当天开始多少分钟
	 * @param currentDate
	 * @return
	 */
	public static Integer getPastTimeOneDay(Date currentDate) {
        LocalDateTime yesterday = LocalDateTime.ofInstant(currentDate.toInstant(),
                ZoneId.systemDefault()).withHour(0).withMinute(0)
                .withSecond(0).withNano(0);
        LocalDateTime currentDateTime = LocalDateTime.ofInstant(currentDate.toInstant(),
                ZoneId.systemDefault());
        long minutes = ChronoUnit.MINUTES.between(yesterday,currentDateTime);
        return (int) minutes;
    }
	/**
     * 
    * @Title: daysDiff
    * @Description: TODO(计算天数差值)
    * @param start	开始时间，2020-12-09
    * @param end	结束时间，2020-12-10
    * @return
    * int
    * @author 刘念
     */
    public static int daysDiff(String start,String end){
        ...
        Date date_b = null;
        try {
            date_a = sdf.parse(start);//字符串转Date
            date_b = sdf.parse(end);
